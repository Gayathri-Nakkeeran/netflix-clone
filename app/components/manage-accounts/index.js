...
export default function ManageAccounts() {
  ...
  async function getAllAccount() {
    ...
    if (data && data.data && data.data.length) {
      ...
      // console.log("from save handler", data.data)
      setPageLoader(false);
    } else {
      setPageLoader(false);
    }
        ...
        <ul className="flex gap-8 p-0 my-[25px]">
          {accounts && accounts.length
            ? accounts.map((item) => (
              <li
                ...
                key={item._id}
                className="max-w-[200px]w-[155px] cursor-ponter flex flex-col items-center gap-3 min-w-[200px]"
              >
                <div className="relative">
                  <Image
                    ...
                    height={200}
                    alt="account"
                    className="max-w-[200px] rounded min-w-[84px] max-h-[200px] min-h-[82px] object-cover w-[155px] h-[155px] "
                  />
                  {showDeleteIcon ? (
                    <div
                      onClick={() => {
          ...
          {accounts && accounts.length < 4 ? (
            <li
              ...
              className="border text-black bg-[#e5b109] font-bold text-lg border-black max-w-[200px] rounded min-w-[84px] max-h-[200px] min-h-[82px]  w-[155px] h-[155px] cursor-pointer flex justify-center items-center "
            >
              Add Account
            </li>
          ) : null}
